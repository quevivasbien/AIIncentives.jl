...
function visual_check(row::DataFrameRow)
    ...
    plot(
        plot_payoffs_with_xs(
            ...
            min_Xp = base_Xp[1] / 10, max_Xp = base_Xp[1] * 2
        ),
        plot_payoffs_with_xs(
            problem, base_Xs, base_Xp, 2,
            min_Xp = base_Xp[2] / 10, max_Xp = base_Xp[2] * 2
        ),
        plot_payoffs_with_xp(
            problem, base_Xs, base_Xp, 1,
            min_Xs = base_Xs[1] / 10, max_Xs = base_Xs[1] * 2
        ),
        plot_payoffs_with_xp(
            problem, base_Xs, base_Xp, 2,
            min_Xs = base_Xs[2] / 10, max_Xs = base_Xs[2] * 2
        ),
        layout = (2, 2)
    )
end